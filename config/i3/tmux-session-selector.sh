...
# Function to build session list
build_session_list() {
    local session_list=()
...
readarray -t reversed_list < <(build_session_list)

# Create fzf selection with preview and delete key support
if [ ${#reversed_list[@]} -gt 1 ]; then
    choice=$(printf "%s\n" "${reversed_list[@]}" | fzf --prompt="Select tmux session (DEL to kill): " \
        --preview '/home/basil/.config/i3/tmux-preview.sh {}' \
        --preview-window=right:50%:wrap \
        --bind "del:execute(/home/basil/.config/i3/tmux-kill-session.sh {} >/dev/null 2>&1)+refresh-preview")
else
    choice=$(printf "%s" "[New Session]" | fzf --prompt="Select tmux session: ")
fi

# Handle --list-only flag for reload
if [ "$1" = "--list-only" ]; then
    build_session_list
    exit 0
fi
...
if [ "$choice" = "[New Session]" ] || [ -z "$choice" ]; then
    ...
    # Debug output
    echo "Attempting to attach to session: '$session_name'"
    ...
    if tmux has-session -t "$session_name" 2>/dev/null; then
        ...
        echo "Session '$session_name' no longer exists. Creating new session instead..."
        sleep 2
        tmux new-session
    fi
fi